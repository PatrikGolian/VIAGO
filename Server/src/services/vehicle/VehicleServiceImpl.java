...
public class VehicleServiceImpl implements VehicleService
{
  ...
  @Override public void addNew(AddNewVehicleRequest request) throws SQLException
  {
    ...
    if (request instanceof AddNewBikeRequest bikeRequest)
    {
      ...
      Bike newBike = new Bike(bikeRequest.id(), bikeRequest.type(),
    ...
    else if (request instanceof AddNewEBikeRequest eBikeRequest)
    {
      ...
      EBike newEBike = new EBike(eBikeRequest.id(), eBikeRequest.type(),
          eBikeRequest.brand(), eBikeRequest.model(), eBikeRequest.condition(),
          eBikeRequest.color(), eBikeRequest.pricePerDay(),
          eBikeRequest.bikeType(), eBikeRequest.maxSpeed(),
          eBikeRequest.oneChargeRange(), eBikeRequest.ownerEmail(),
          eBikeRequest.state());
    ...
    else if (request instanceof AddNewScooterRequest scooterRequest)
    {
      ...
      Scooter newScooter = new Scooter(scooterRequest.id(),
          scooterRequest.type(), scooterRequest.brand(), scooterRequest.model(),
          scooterRequest.condition(), scooterRequest.color(),
          scooterRequest.pricePerDay(), scooterRequest.maxSpeed(),
          scooterRequest.oneChargeRange(), scooterRequest.ownerEmail(),
    ...
    else
    {
      throw new IllegalArgumentException(
          "Unknown vehicle request type: " + request.getClass());
    }
  ...
  {
    if (!(type.equals("bike") || type.equals("e-bike") || type.equals(
        "scooter")))
    {
