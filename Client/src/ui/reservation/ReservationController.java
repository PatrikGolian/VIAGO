...
public class ReservationController implements Controller
{
  ...
  public void initialize()
  {
    ...
    viewModel.selectedVehicleProperty().bind(vehicleTable.getSelectionModel().selectedItemProperty());
    viewModel.selectedVehicleProperty().addListener((obs, oldValue, newVal) ->
    {
      viewModel.setVisibility();
      viewModel.conditionProperty().set(newVal.conditionPropProperty().get());
      viewModel.colorProperty().set(newVal.colorPropProperty().get());
      viewModel.getOwnerEmailProp().set(newVal.ownerPropProperty().get());
      switch (newVal.typePropProperty().get())
      {
        case "scooter" ->
        {
          viewModel.bikeTypeProperty().set("");
          viewModel.speedProperty().set(newVal.maxSpeedPropProperty().get());
          viewModel.rangeProperty().set(newVal.rangeProperty().get());
        }
        case "e-bike" ->
        {
          viewModel.bikeTypeProperty().set(newVal.bikeTypePropProperty().get());
          viewModel.speedProperty().set(newVal.maxSpeedPropProperty().get());
          viewModel.rangeProperty().set(newVal.rangeProperty().get());
        }
        case "bike" ->
    ...
    // Visibility
    conditionField.visibleProperty().bind(viewModel.conditionFieldVisibilityProperty());
    conditionLabel.visibleProperty().bind(viewModel.conditionLabelVisibilityProperty());
    ...
    ownerField.visibleProperty().bind(viewModel.ownerEmailFieldVisibilityProperty());
    ownerLabel.visibleProperty().bind(viewModel.ownerEmailLabelVisibilityProperty());
    ...
    datePicker.showingProperty().addListener((obs, oldValue, newValue) -> {
      if (newValue)
      {
        ...
        List<DateCell> cells = rawCells.stream()
            .filter(n -> n instanceof DateCell)
            .map(n -> (DateCell) n)
